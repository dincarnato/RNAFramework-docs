...
<br/><br/>
Since version __2.9.1__, RF Norm allows generating plots of raw RT-stop counts (methods 1-2) or mutation rates (methods 3-4), hence enabling quick visual QC of probing experiments:
...
__-i__ *or* __--index__ | string | An RCI index file to be used for all input RC files<br/>__Note #1:__ If no RCI index is provided, RF Norm will look for files with .rci extension in the same input folder as the RC files, named after the RC files (e.g., `Sample.rc` will look for `Sample.rc.rci`).<br/>__Note #2:__ If no RCI file is found, it will be created at runtime, and stored in the same folder of the input RC files.
...
__-g__ *or* __--img__ | | Enables the generation of plots of raw reactivity data (RT-stops or mutation rates) for each transcript across all samples (requires R)
